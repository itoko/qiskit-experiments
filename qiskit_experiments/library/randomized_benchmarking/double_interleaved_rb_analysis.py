...
class DoubleInterleavedRBAnalysis(RBAnalysis):
    ...
    def _generate_fit_guesses(
        ...
        int_curve = self._data(series_name="Interleaved-First")
        opt_int1 = user_opt.copy()
        if opt_int1.p0["alpha_c1"] is not None:
            opt_int1.p0["alpha"] = opt_std.p0["alpha"] * opt_int1.p0["alpha_c1"]
        opt_int1 = self._initial_guess(opt_int1, int_curve.x, int_curve.y, self._num_qubits)

        int_curve = self._data(series_name="Interleaved-Second")
        opt_int2 = user_opt.copy()
        if opt_int2.p0["alpha_c1"] is not None:
            opt_int2.p0["alpha"] = opt_std.p0["alpha"] * opt_int2.p0["alpha_c1"]
        opt_int2 = self._initial_guess(opt_int2, int_curve.x, int_curve.y, self._num_qubits)

        user_opt.p0.set_if_empty(
            a=opt_std.p0["a"],
            alpha=opt_std.p0["alpha"],
            alpha_c1=min(opt_int1.p0["alpha"] / opt_std.p0["alpha"], 1),
            alpha_c2=min(opt_int2.p0["alpha"] / opt_std.p0["alpha"], 1),
            b=opt_std.p0["b"],
        )
    ...
    def _extra_database_entry(self, fit_data: curve.FitData) -> List[AnalysisResultData]:
        ...
        extra_data1 = AnalysisResultData(
            name="EPC1",
            value=epc1,
        ...
        extra_data2 = AnalysisResultData(
            name="EPC2",
            value=epc2,
            chisq=fit_data.reduced_chisq,
            quality=self._evaluate_quality(fit_data),
